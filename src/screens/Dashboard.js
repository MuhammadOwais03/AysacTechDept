...
// How far each card is offset from the left based on distance from active
// offset 0 = active (full width, marginLeft: 20)
// offset ±1 = slightly indented
// offset ±2 = more indented
// This creates the arc/curve look from the Figma
const LEFT_INDENT = [20, 60, 90, 110, 120];
...
export default function Dashboard() {
  ...
  const getCardMarginLeft = (index) => {
    ...
    const clampedDist = Math.min(distance, LEFT_INDENT.length - 1);
    return LEFT_INDENT[clampedDist];
  };

  const getCardOpacity = (index) => {
    const distance = Math.abs(index - activeIndex);
    if (distance === 0) return 1;
    return Math.max(0.4, 1 - distance * 0.18);
  };
  ...
  const renderItem = ({ item, index }) => {
    const isActive = index === activeIndex;
    const marginLeft = getCardMarginLeft(index);
    const opacity = getCardOpacity(index);

    if (item.isAdd) {
      return (
        <View style={[styles.addCard, isActive && styles.projectCardActive, { marginLeft, opacity }]}>
          <TouchableOpacity style={styles.addInner} activeOpacity={0.8}>
            <View style={styles.addCircle}>
              <Text style={styles.addPlus}>+</Text>
            </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.projectsSection}>
        ...
        {/* CARD LIST — full width, orb shows through */}
        <FlatList
          ref={flatListRef}
          data={PROJECTS}
          renderItem={renderItem}
...
const styles = StyleSheet.create({
  ...
  // BASE CARD — marginLeft is set dynamically per item
  projectCard: {
    height: 85,
    flexDirection: 'row',
    alignItems: 'center',
    ...
    paddingHorizontal: 16,
    marginBottom: 6,
    // marginLeft applied inline per card based on distance from active
  },
  projectCardActive: {
    backgroundColor: 'rgba(255,255,255,0.92)',
  },
